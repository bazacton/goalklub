...
$header_style = '';
  	...
  	if(is_page()){
		...
		if(isset($cs_page_options) and $cs_page_options != 'default' and $cs_page_options <> ''){
			...
			$page_setting=unserialize((function_exists('base_64_decode')) ? base_64_decode($settings) : "");
...
<html <?php language_attributes(); ?>>
   ...
   <?php
	...
	<body <?php body_class($cs_rtl_class);  if($cs_site_layout !='full_width'){ if ( function_exists( 'cs_bg_image' ) ) { echo cs_bg_image(); } } ?>  <?php if(isset($cs_theme_options['cs_smooth_scroll']) and $cs_theme_options['cs_smooth_scroll'] == 'on'){ ?>style="overflow: hidden;"<?php } ?>>
	...
	<?php
 		if($header_style == 'custom_slider' && $slider_position == 'above'){
			...
			if(isset($cs_theme_options['cs_announcment_switch']) and $cs_theme_options['cs_announcment_switch']=='on'){
