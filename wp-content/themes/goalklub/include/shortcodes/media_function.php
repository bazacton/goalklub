...
if (!function_exists('cs_video_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_video', 'cs_video_shortcode');
    }
}
...
if (!function_exists('cs_image_shortcode')) {
    ...
    function cs_image_shortcode($atts, $content = "") {
        $defaults = array('column_size' => '', 'cs_image_section_title' => '', 'image_style' => '', 'cs_image_url' => '#', 'cs_image_link' => '', 'cs_image_title' => '', 'cs_image_caption' => '', 'cs_image_custom_class' => '', 'cs_image_custom_animation' => '');
        extract(shortcode_atts($defaults, $atts));
        ...
        $html.= $section_title;
        $html .='<div class="frtable"><div class="frtable-row">';

        if (isset($cs_image_url) && $cs_image_url != '') {
            
            $cs_image_link = "'" .$cs_image_link. "'";
            $html .= '<div class="frtabel-cell imageframe"><a data-title="' . $cs_image_title . '" onclick="goto_func('.$cs_image_link.');"  href="javascript:void(0);" data-rel="prettyPhoto">';
            $html .= '<img alt="No image" src="' . esc_url($cs_image_url) . '">';
            $html .= '</a></div>';
        }

        $html .= '<div class="frtabel-cell"><section>';
        ...
        $html .= '</section></div>';
        $html .= '</div></div></div>';
        $output.='<div ' . $CustomId . '>' . $html . '</div>';

        return do_shortcode($output);
    }
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_image', 'cs_image_shortcode');
    }
}
...
if (!function_exists('cs_promobox_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_promobox', 'cs_promobox_shortcode');
    }
}
...
if (!function_exists('cs_slider_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_slider', 'cs_slider_shortcode');
    }
