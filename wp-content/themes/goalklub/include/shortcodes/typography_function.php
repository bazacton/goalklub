...
if (!function_exists('cs_column_shortocde')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_column', 'cs_column_shortocde');
}
}
// adding column end
...
if (!function_exists('cs_tooltip_shortcode')) {
	function cs_tooltip_shortcode($atts, $content = "") {
		...
		<span class="tolbtn btn-default custom-btn" data-toggle="tooltip" data-placement="'.$tooltip_data_placement.'" title="'.$tooltip_hover_title.'" style="background-color:#fff; color:#fff; box-shadow:0px 4px 4px -2px rgba(0, 0, 0, 0.5);">'.$tooltip_hover_title.'</span>';
					
		return do_shortcode('<div class="'.$column_class.$cs_tooltip_animation.'">'.$html.'</div>');
	}
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_tooltip', 'cs_tooltip_shortcode');
}
}
// adding Tooltip end
...
if (!function_exists('cs_dropcap_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_dropcap', 'cs_dropcap_shortcode');
}
}
// adding dropcap end
...
if (!function_exists('cs_divider_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_divider', 'cs_divider_shortcode');
}
}
...
if (!function_exists('cs_quote_shortcode')) {
	function cs_quote_shortcode( $atts, $content = null ) {
		...
		$html	.= '<blockquote class="cs-qoute '.$cs_quote_class.' '.$quote_align.' '.$cs_quote_animation.'" '.$customId.'  style="animation-duration: '.$cs_quote_animation_duration.'s; color:'.$quote_text_color.'">' . do_shortcode($content) . $author_name .'</blockquote>';
		
		return '<div class="'.$column_class.'">'.$section_title.$html.'</div>';
	}
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_quote', 'cs_quote_shortcode');
}
}
...
if (!function_exists('cs_hightlight_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_highlight', 'cs_hightlight_shortcode');
}
}
...
if (!function_exists('cs_heading_shortcode')) {
	function cs_heading_shortcode($atts, $content = "") {
		...
		$html .= '<div  class="'.$heading_animation.'" >';
			...
			if($heading_divider == 'on' and $heading_align <> 'center'){
				$html	.= '<div class="box_spreater"><div class="spreater " style="text-align: '.$heading_align.';"></div></div>';
			}elseif($heading_divider == 'on' and $heading_align=='center'){
				$html .='<div class="box_spreater"><div class="spreater"><div class="spreater-holder"><span></span></div></div></div>';
			}
			if($content <> ''){
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_heading', 'cs_heading_shortcode');
}
}
// adding heading end
...
if (!function_exists('cs_list_shortcode')) {
	function cs_list_shortcode($atts, $content = "") {
		...
		$html	.= '<div '.$customID.' class="'.$column_class.' '.$cs_list_animation.' '.$cs_list_class.' cs-simple-list">';
		$html	.= $section_title;
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_list', 'cs_list_shortcode');
}
}
...
if (!function_exists('cs_list_item_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('list_item', 'cs_list_item_shortcode');
}
}
...
if (!function_exists('cs_contactus_shortcode')) {
	function cs_contactus_shortcode($atts, $content = "") {
        ...
        <script type="text/javascript">
			function frm_submit<?php echo esc_js($cs_email_counter);?>(){
				...
				var $ = jQuery;
				$("#loading_div<?php echo esc_js($cs_email_counter);?>").html('<img src="<?php echo esc_js(esc_url(get_template_directory_uri()));?>/assets/images/ajax-loader.gif" alt="No Image" />');
		...
		if ( isset( $cs_contactus_label ) && $cs_contactus_label == 'on' ) {
			$html	.= '<label>'.__('Enter Your Name','goalklub').'</label>';
		}
		...
		if ( isset( $cs_contactus_label ) && $cs_contactus_label == 'on' ) {
			$html	.= '<label>'.__('Enter Your Email Address','goalklub').'</label>';
		}
		...
		if ( isset( $cs_contactus_label ) && $cs_contactus_label == 'on' ) {
			$html	.= '<label>'.__('Enter Subject','goalklub').'</label>';
		}
		...
		if ( isset( $cs_contactus_label ) && $cs_contactus_label == 'on' ) {
			$html	.= '<label>'.__('Message','goalklub').'</label>';
		}
		
		$html	.= '<textarea class="'.$class.' '.$view_class.'" placeholder="Message" name="contact_msg" required></textarea>';
		$html	.= '<input type="submit" value="'.__('Submit Now','goalklub').'" class="custom-btn cs-bg-color" id="submit_btn'.$cs_email_counter.'">';
		$html	.= '</form>';
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_contactus', 'cs_contactus_shortcode');
}
}
...
if (!function_exists('cs_message_shortcode')) {
	function cs_message_shortcode($atts, $content = "") {
		...
		if(isset($cs_message_type) && $cs_message_type == 'alert'){
			if(isset($cs_style_type) && $cs_style_type == 'fancy'){
				...
				$html .= '<button data-dismiss="alert" class="close-v1" type="button"><em class="icon-cross3"></em></button>';
				}
			...
			<div style="background:'.$cs_background_color.';  box-shadow:0px 4px 3px 0px #e0e0e0; margin-bottom:25px;" class="messagebox-v3 alert alert-info has_pattern icon_position_left no_border">';
				if(isset($cs_message_close) && $cs_message_close == 'yes'){
				$html .= '<button data-dismiss="alert" class="close-v2" type="button"><em class="icon-cross3"></em></button>';
				}
				if(isset($cs_message_icon) && $cs_message_icon <> ''){
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add('cs_message', 'cs_message_shortcode');
}
}
// adding message end
...
if (!function_exists('cs_testimonials_shortcode')) {
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add( 'cs_testimonials', 'cs_testimonials_shortcode' );
}
}
...
if (!function_exists('cs_testimonial_item')) {
	function cs_testimonial_item( $atts, $content = null ) {
		...
		if(isset($testimonial_img) && $testimonial_img <> ''){
			$figure = '<figure><img src="'.$testimonial_img.'" alt="No image" /></figure>';
		}
		$tc_color = '';
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add( 'testimonial_item', 'cs_testimonial_item' );
}
}
