 ...
 function cs_googlefont_list(){
	...
	$value = 'true';
 	...
 	if($check_transient === false){
  		$get_response = wp_remote_get($google_api_url, array('sslverify' => false,'decompress' => false));
...
function cs_get_google_fontlist($response = ''){
	...
	//$response = WP_Filesystem ($cachefile);
	$font_list= array();
	$i=0;
	if(is_array($items)){
	...
	function cs_get_font_family($font_index = 'default', $att = 'regular') {
		...
		if($font_index != 'default'){
			...
			if(isset($fonts) and is_array($fonts)){
				...
				$html =  wp_enqueue_style('url_css_dynamic', $url); //'@import url('.$url.');';
			}
		}
		else{
			$html = '';
