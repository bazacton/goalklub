        ...
        if (isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG) {
            ...
            if ('SimpleXML_parse_error' == $result->get_error_code()) {
                foreach ($result->get_error_data() as $error)
                    echo force_back($error->line . ':' . $error->column . ' ' . esc_html($error->message) . "\n");
            } else if ('XML_parse_error' == $result->get_error_code()) {
                $error = $result->get_error_data();
                echo force_back($error[0] . ':' . $error[1] . ' ' . esc_html($error[2]));
            }
            echo '</pre>';
            echo '<p><strong>' . __('There was an error when reading this WXR file', 'goalklub') . '</strong><br />';
            echo __('Details are shown above. The importer will now try again with a different parser...', 'goalklub') . '</p>';
        }
...
class WXR_Parser_SimpleXML {
    ...
    function parse($file) {
        ...
        if (!$success || isset($dom->doctype)) {
            ...
            return new WP_Error('SimpleXML_parse_error', __('There was an error when reading this WXR file', 'goalklub'), libxml_get_errors());
        }
        ...
        if (!$wxr_version)
            return new WP_Error('WXR_parse_error', __('This does not appear to be a WXR file, missing/invalid WXR version number', 'goalklub'));
...
class WXR_Parser_XML {
    ...
    function parse($file) {
        ...
        if (!xml_parse($xml, WP_Filesystem($file), true)) {
...
class WXR_Parser_Regex {
    ...
    function fopen_($filename, $mode = 'r') {
        if ($this->has_gzip)
            return gzopen($filename, $mode);
        return WP_Filesystem($filename);
    }
